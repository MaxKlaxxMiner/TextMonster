...
namespace TextMonsterSystem
{
  ...
  public abstract class ITextMonster : IDisposable
  {
    ...
    public virtual IEnumerable<char> GetChars(LinePos linePos, long charOffset)
    {
      ...
      long charCount = GetCharPos(linePos.lineEnd) - GetCharPos(linePos.lineStart);

      return GetChars(linePos.lineStart).Take((int)charCount).Skip((int)charOffset);
    }
    #endregion
    ...
    public virtual LinePos GetNextLine(LinePos linePos)
    {
      ...
      if (p < Length && GetChars(GetMemoryPos(p)).First() == '\r') p++;
    ...
    public virtual LinePos GetPrevLine(LinePos linePos)
    {
      ...
      if (p > 0 && GetChars(GetMemoryPos(p - 1)).First() == '\r') p--;
