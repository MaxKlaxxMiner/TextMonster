...
namespace TextMonster.Xml.Xml_Reader
{
  ...
  public class XmlSchemaImporter : SchemaImporter
  {
    ...
    MembersMapping ImportMembersType(XmlSchemaType type, string typeNs, string identifier)
    {
      ...
      MemberMapping[] members = ImportTypeMembers(type, typeNs, identifier, memberScope, new CodeIdentifiers(), new NameTableScope(), ref needExplicitOrder, false, false);
    ...
    {
      NameTableScope choiceElements = new NameTableScope();
    ...
    bool GatherGroupChoices(XmlSchemaGroup group, NameTableScope choiceElements, string identifier, string ns, ref bool needExplicitOrder, bool allowDuplicates)
    {
      return GatherGroupChoices(group.Particle, choiceElements, identifier, ns, ref needExplicitOrder, allowDuplicates);
    }

    bool GatherGroupChoices(XmlSchemaParticle particle, NameTableScope choiceElements, string identifier, string ns, ref bool needExplicitOrder, bool allowDuplicates)
    {
      if (particle is XmlSchemaGroupRef)
      {
        XmlSchemaGroupRef refGroup = (XmlSchemaGroupRef)particle;
