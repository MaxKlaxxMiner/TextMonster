...
namespace TextMonster.Xml.Xml_Reader
{
  ...
  public class XmlReflectionImporter
  {
    ...
    NameTableScope types = new NameTableScope();      // xmltypename + xmlns -> Mapping
    NameTableScope nullables = new NameTableScope();  // xmltypename + xmlns -> NullableMapping
    NameTableScope elements = new NameTableScope();   // xmlelementname + xmlns -> ElementAccessor
    NameTableScope xsdAttributes;   // xmlattributetname + xmlns -> AttributeAccessor
    Hashtable specials;   // type -> SpecialMapping
    Hashtable anonymous = new Hashtable();   // type -> AnonymousMapping
    NameTableScope serializables;  // type name --> new SerializableMapping
    StructMapping root;
    string defaultNs;
    ...
    Accessor ReconcileAccessor(Accessor accessor, NameTableScope accessors)
    {
        ...
        if (attrs.Length > 0)
        {
          ...
          if (qname != null && !qname.IsEmpty)
          {
            if (serializables == null)
              serializables = new NameTableScope();
    ...
    TypeMapping GetTypeMapping(string typeName, string ns, TypeDesc typeDesc, NameTableScope typeLib, Type type)
    {
      TypeMapping mapping;
    ...
    MembersMapping ImportMembersMapping(XmlReflectionMember[] xmlReflectionMembers, string ns, bool hasWrapperElement, bool rpc, bool openModel, RecursionLimiter limiter)
    {
      ...
      NameTableScope elements = new NameTableScope();
      NameTableScope attributes = new NameTableScope();
    ...
    {
      NameTableScope arrayItemElements = new NameTableScope();   // xmlelementname + xmlns -> ElementAccessor
    ...
    void ImportAccessorMapping(MemberMapping accessor, FieldModel model, XmlAttributes a, string ns, Type choiceIdentifierType, bool rpc, bool openModel, RecursionLimiter limiter)
    {
      ...
      if (accessor.TypeDesc.IsArrayLike)
      {
        ...
        if ((flags & attrFlags) != 0)
        {
          ...
          if (attribute.Form == XmlSchemaForm.Qualified && attribute.Namespace != ns)
          {
            if (xsdAttributes == null)
              xsdAttributes = new NameTableScope();
        ...
        else if ((flags & elemFlags) != 0)
        {
          ...
          NameTableScope anys = new NameTableScope();
    ...
    void CheckAmbiguousChoice(XmlAttributes a, Type accessorType, string accessorName)
    {
      ...
      {
        NameTableScope arrayTypes = new NameTableScope();
    ...
    {
      NameTableScope ids = new NameTableScope();
